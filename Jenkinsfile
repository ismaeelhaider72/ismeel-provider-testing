        ...
        stage ('Testing') {            
          steps {                
                script {
                    ...
                    try{    
                    stack = sh("aws cloudformation describe-stacks --stack-name ismaeelawsclitest2  --region us-east-1  --query Stacks[0].StackStatus --output text ", returnStdout: true ) 
                    echo "ismaeeeeeelllllllllllll"
                    echo stack
                    stack=true
                    }
