        ...
        stage ('Testing') {            
          steps {                
                script {
                    ...
                    status = sh(script: "aws cloudformation describe-stacks --stack-name ismaeelawsclitest2 --query Stacks[0].StackStatus --output text --region us-east-1 ", returnStdout: true)
                    sh "echo --------------------" 
                    sh "echo ${status}"   
