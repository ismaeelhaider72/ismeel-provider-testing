        ...
        stage ('Testing') {            
          steps {                
                script {
                  ...
                  // stack not existed
                    GIT_COMMIT_EMAIL = sh ( script: 'aws cloudformation describe-stacks --stack-name ismaeelawsclitest2 --query Stacks[0].StackStatus --region us-east-1',returnStdout: true ).trim()
                    echo "-------------------------"    
